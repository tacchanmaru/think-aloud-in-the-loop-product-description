...
export default function BaselineLLM() {
  ...
  // プロンプト入力欄の高さを自動調整する関数
  const adjustPromptHeight = () => {
    if (promptRef.current) {
      promptRef.current.style.height = '0'
  ...
  const handleEditDescription = async () => {
    if (!userId || !text || !prompt) {
      toast({
        title: "入力エラー",
        description: "テキストとプロンプトを入力してください。",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      const response = await fetch('http://localhost:8000/api/edit-description', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        setText(result.edited_description)
        setPrompt("") // プロンプトをクリア
        setHasLLMEdit(true) // LLMでの編集が成功したことを記録
      } else {
        const errorMessage = `テキストの編集に失敗しました: ${result.error}`
        setApiError(errorMessage)
        toast({
          title: "エラー",
          description: errorMessage,
            ...
            {mode === "upload" ? (
              ...
              <>
                ...
                <div className="space-y-2">
                  <p className="text-sm font-medium">商品説明文：</p>
                  <div 
                    className="w-full p-3 bg-gray-50 rounded-md whitespace-pre-wrap break-words border"
                  >
                    {text || "商品説明文がありません"}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <p className="text-sm font-medium">プロンプト：</p>
                  <Textarea
                    ref={promptRef}
                    placeholder="商品説明文の編集指示を入力してください..."
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    className="min-h-[1.5em] resize-none overflow-hidden"
                  />
                  <div className="space-y-2">
                    <Button 
                      onClick={handleEditDescription}
                      disabled={isProcessing || !text || !prompt}
                      className="w-full"
                    >
                      {isProcessing ? (
                        <>
                    ...
                    <div className="flex justify-end">
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button
                            variant={hasLLMEdit ? "default" : "secondary"}
                            className={hasLLMEdit 
                              ? "bg-blue-600 hover:bg-blue-700 transition-colors" 
                              : "bg-gray-200 text-gray-500 cursor-not-allowed"}
                            disabled={!hasLLMEdit}
                          >
